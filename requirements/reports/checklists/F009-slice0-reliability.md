...
## C03 — 숫자 검증 (2026-09-24)

영역: `SRV-REQ-037.md` C03 절

| 확인 | 결과 |
|---|---|
| 진단 재현 4문장(12% · 방향 반대 · 근거 없는 인과 · 단위 다름) | 옛 검사 전부 통과 → **전부 막음** |
| 정상 문장(반올림 · 같은 방향 · 세는 말 · 억/만 환산 · 근거 있는 인과) | 통과 |
| 서버 | build · test **361 / 0** · eslint · layer-check |

## C01 — 해설 사실을 서버가 조립 (2026-09-24)

영역: `SRV-REQ-025.md` §13 · `FE-REQ-028.md` C01 절 · `BFF-REQ-025.md` C01 절

| 확인 | 결과 |
|---|---|
| 요청 | `{ symbol, mode }` — 옛 본문 필드는 버린다 |
| 사실 | 게이트와 같은 재료(시세 · 지표 · 심리 · 대량 체결) + 서버 종목 뉴스, 응답 `facts.hash` |
| 위조 | 요청에 실은 가격 · 근거가 해설 입력에 없다 |
| 서버 | build · test **367 / 0** · eslint · layer-check |
| BFF | 코드 변경 없음 · build · test **121 / 0** |
| 프론트 | check-types · lint · test · build(web · web-tax) · layer-check |
...
| 사실 `metricId` 구조 · 스냅샷 ID | 근거가 라벨 + 문자열 · 화면이 보내는 사실 0 | 도구 호출 에이전트(`FEATURE-008` FR-41 · 44) |
| 숫자 검증의 지표 이름 대조 · 새 규칙으로 버려지는 LLM 문장 비율 | 사실 구조 없음 · 안 쟀다 | C01 · 관측성 계측 |
