...
| 확인 | 결과 |
|---|---|
| 로그인 렌더 | 빌드 HTML — `<input>` 2(`aria-label` 이메일 · 비밀번호) · `<button>` 1 · `<label>` 0 · h1 "로그인" |
| 배경 메시 | 빌드 CSS 에 `radial-gradient` 3겹 · 카드 `border-radius: 20px` |
| 상세 헤더 | 지표가 한 줄에 하나씩 쌓이던 것 → 가로 흐름 + 칸 사이 세로선. 가격 t1 · 이름 t5 |
| 표 마감 | 값 오른쪽 정렬 · `tabular` · t6 bold, 보조 문구 t8 tertiary, 행 구분선 `neutral[100]` |
| 관찰 구간 띠 | 채움 14% + 경계선 + 채운 이름표. 상세 · 패널 **같은 값** |
| 보드 잘림 | 상자를 `sticky` 로 — 스크롤 복원 상태에서도 표 머리 · 패널 위쪽이 남는다 |
| 게이트 | `check-types` · `lint`(monorepo) · `build`(worktree) · `pnpm test` 63 |
| 사용자 확인 | **2026-09-23 "너무 좋다"** — 로그인 · 상세 · 띠 · 돌아오기 |
...
| 참고 화면과의 **치수** 비교 | Chrome 확장이 연결되지 않아 두 화면을 나란히 놓고 재지 못했다. 사용자 스크린샷으로 3회 반복해 맞췄다 | 확장 연결 시 |
| 보유 종목에서 관찰 구간 띠 | 서버가 `held_rule` 과 `observation` 중 **하나만** 준다 | 서버 계약 변경 시 |
