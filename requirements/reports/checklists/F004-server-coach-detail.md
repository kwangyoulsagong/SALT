...
| 인증된 HTTP 200 | 로컬 토큰 발급이 세션 권한에서 막혔다. 유스케이스를 실제 어댑터로 불러 대신했다 | BFF `/api/app/coach/report` 슬라이스 |
...
| BFF 전달 | BFF 가 상세를 부르지 않는다. `behavior-coach` 카드 매핑이 `factCode` · `params` 를 떨군다 | `BFF-REQ-023` |
