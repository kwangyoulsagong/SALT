...
## 후속 — 사용자 검수 왕복 (2026-09-23, 같은 브랜치)

| 판단 | 내용 |
|---|---|
| 진입 | 리포트는 **투자 화면** 제목 줄에서 들어온다(홈 아님). 상세는 표 행 클릭 · 종목 이름 링크(패널 버튼 삭제) |
| 디자인 | 참고 화면 12페이지를 실제로 열어 계산 스타일을 뽑았다 → `shared/ui/surface.css`. 종목이 나오는 모든 자리에 로고 |
| 목 | **MSW 전부 삭제**(`FE-REQ-012`). 목표는 BFF 실경로, 서버에 없는 지출 막대는 삭제 |
| SEO | 공개(상세 · 투자) = 서버 머리 · 메타데이터 · JSON-LD · sitemap · robots, 개인 = noindex |
| 번들 | 서버 컴포넌트가 슬라이스 배럴을 값으로 import 하지 않는다(161 → 113 kB) — 규칙 `performance-frontend.md` |

커밋: `a0e8631` 재설계 · `7640d11` 진입 · `dcbd7a9` Button weak · `0fb30fc` FilterTabs chip · `f7606d7` BFF logoUrl ·
`8f3a2d9` 실측 디자인 · `478abcd` 목 규칙 · `7b2663a` 목 제거(BREAKING — 개발도 BFF 필요) · `42d4867` 행 클릭 ·
`e169671` 이름 링크 · `f2c342f` SEO · `d7633ac` 머리 서버 컴포넌트
