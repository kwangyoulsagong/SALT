...
> **개정 2026-09-21 — `ADR-002`.** 원장 · 청구서 · 세금(FEATURE-001 · 002)을 뺐다
> (*"우린 돈을 잘 버는 걸 도와주는 거지"*). 아래 §0 의 네 가지 중 **마감일 · 회계**, §3 의
> "회계에 걸어야 하는 이유", §4 의 세금 엔진, §9 · §10 의 P1 · P2 순서와 하드 마감은 **기록으로만**
> 남는다. 지금 제품은 **추천(F004) · 한 숫자(F003) · 코치 대화와 3탭(F006)** 이고, 보유 기록은
> 기존 `PortfolioTransaction` 으로 한다. 스토리보드 대조 결과와 결정은
> `pm/requirements/reports/feature-audits/2026-09-21-storyboard-gap.md`.

## 0. 한 줄 정의

> **SALT는 "지금 뭘 해야 하는지"를 한 화면에서 알려주는 개인용 투자 코치다. 추천에는 항상 근거·과거 적중률·틀렸던 사례가 붙고, 내 과거 결정의 값은 원화로 계산해서 보여준다.**

돈은 SALT에 들어오지 않는다. 매매는 업비트/증권사에서 직접 하고, **계좌를 연동하지 않고 얼마에 얼마나 넣었는지 직접 입력한다.** SALT가 하는 일은 네 가지다.
...
> **2026-09-23 `ADR-003`** 이 아래 "목표주가 / 수익률 예측" 행과 1-3 의 "목표주가·수익률 예측 금지"를 대체한다 —
> 확률 · 구간 전망을 채점 3종과 함께 **소유자 계정에만** 연다. 한 점 목표가 · 명령형 지시 · 확신 표현은 계속 금지.
| 안 함 | 왜 |
|---|---|
| 주문 실행 / 자동매매 / 자금 보관 / 출금 | 금융투자업 요건에 걸리지 않기 위해 |
| **거래소·증권사 계좌 연동 (API 키 · CSV import)** | **사용자 결정 2026-09-08.** 원장은 100% 수기 입력. API 키를 아예 받지 않으므로 키 유출·권한 오용 리스크가 원천적으로 없다 |
...
## 8. 데이터 소스 — 원장은 수기, 시세만 조회

**사용자 결정(2026-09-08): 계좌를 연동하지 않는다.** 얼마에 얼마나 투자했는지 직접 입력한다.

| 구분 | 소스 | 인증 | 비고 |
|---|---|---|---|
| **거래 원장** (수량·단가·수수료·거래일) | **사용자 수기 입력** | — | 유일한 입력 경로. `거래 기록 추가` 화면이 제품의 핵심이 된다 |
| 크립토 시세 | 업비트 **공개** 시세 API / WebSocket | **불필요** | 현재 이미 이 경로로 동작 중 |
| 국내·미국주식 시세 | **미정 (Open Question)** | 소스에 따라 다름 | 아래 8-1 |
| 환율 (USD/KRW) | 기준환율·재정환율 소스 | 무료 소스 존재 | 세금 계산에 필수 |
| 밸류에이션 지표 | MVRV Z · Puell · CAPE | 소스에 따라 다름 | 일 1회 갱신으로 충분 |

### 이 결정이 바꾸는 것

| 항목 | 이전 계획 | 변경 |
|---|---|---|
| 원장 확보 | 업비트 CSV import + 조회 전용 API 키 + KIS 연동 | **전부 제거.** 수기 입력만 |
| 병목(S1) | CSV 파서 · 중복 제거 · 잔고 대조 | **입력 UX와 입력값 검증**. 파서가 아니라 폼이 병목 |
| 보안 표면 | API 키 암호화 저장 · 스코프 검사 · 키 유출 대응 | **없음.** 저장할 키가 없다 |
| 정확도 책임 | 거래소 데이터가 정답 | **사용자 입력이 정답.** 그래서 입력 부담을 줄이고 이상값을 잡는 것이 정확도 전략이 된다 |
| 착수 전 선결 | 업비트 CSV 실물 컬럼 스펙 | **불필요.** 대신 주식 시세 소스 결정이 새 선결 과제 |

### 8-1. 주식 시세 소스 (Open Question — 착수 전 결정 필요)

크립토는 인증 없는 공개 API가 있어 문제가 없지만, **국내·미국주식 시세는 그렇지 않다.** 선택지는 셋이다.

| 안 | 방식 | 장점 | 단점 |
|---|---|---|---|
| **A. 현재가도 수기 입력** | 사용자가 평가금액을 직접 갱신 | 외부 의존 0. 즉시 구현 | 갱신을 안 하면 모든 화면이 낡는다. 청구서·세금이 무의미해짐 |
| **B. 무료 시세 API** | 해외주식은 무료 티어 API, 국내주식은 공개 데이터 | 자동 갱신 | 무료 티어 한도·안정성·약관 확인 필요 |
| **C. 증권사 오픈API (시세 전용)** | 앱키 발급은 하되 **계좌 조회는 쓰지 않음** | 국내·미국 모두 커버 | "계좌 연동 안 한다"는 결정과 경계가 모호해진다 |

> **권장: B.** 시세는 조회이고 계좌가 아니다. C는 앱키가 계좌에 묶여 있어 사용자 결정의 취지와 어긋난다.
> A는 크립토만 있을 때는 성립하지만 3자산군에서는 유지되지 않는다.
> **B의 소스가 확정되기 전까지는 주식 자산군의 현재가를 수기 입력으로 두고(A), 확정 후 자동으로 바꾼다.**
...
| S1 | **원장 입력 UX** — 거래 기록 입력 폼, 입력값 검증, 홀딩 재계산, 환율 원장. **파서가 아니라 폼이 병목** | FEATURE-001 Phase 1 | S0 |
...
> 계좌 연동이 없으므로 정확도는 **입력 편의성**에 달려 있다 — 입력이 번거로우면 사용자가 안 넣고, 안 넣으면 제품이 동작하지 않는다.
...
- [ ] 모든 신규 기능이 **주문을 실행하지 않는다.**
- [ ] **API 키를 저장하는 코드가 0건이다.** 거래소·증권사 계좌에 접속하지 않는다.
- [ ] 거래 1건을 30초 이내에 입력할 수 있다(수기 입력이 유일한 경로이므로).
...
| 원장 부정확 → 전 기능 신뢰 붕괴 | S1을 병목으로 명시. 계좌 연동이 없으므로 **입력값 검증**(이상값 경고·평단 변화 미리보기)과 **사용자 신고 잔고와의 대조**로 잡는다 |
| **입력 귀찮아서 안 넣음** → 제품이 동작하지 않음 | 입력 필드를 최소화(수량·단가·거래일). 현재가·수수료는 자동/생략 허용. 여러 건을 한 번에 넣는 경로 제공. **거래 1건 30초** 목표 |
| 지표 과신(2025-10 톱에서 온체인 지표 전부 실패) | 실패 이력 병기 의무화 |
| 청구서가 자책 도구가 됨 | 잘한 개입도 같은 비중으로 표시(FEATURE-001 FR-11). 점수·등급·벌점 금지 |
| 주식 시세 소스 미정 | 8-1의 A안(수기 입력)으로 시작하고 B안 확정 후 자동 전환. 시세가 없어도 **원장과 세금 계산은 동작**하게 설계 |
...
| 2026-09-08 | **계좌 연동 전면 제외 (사용자 결정).** 원장은 100% 수기 입력. CSV import·Upbit API 키·KIS 연동 제거, 보안 표면 소멸, 병목이 파서 → 입력 UX로 이동. 주식 시세 소스를 새 Open Question으로(8-1) |
