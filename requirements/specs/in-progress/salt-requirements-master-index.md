...
**서버는 1~2단계다.** `SRV-REQ-006`의 `shared` Kernel과 강제 수단이 섰고 안전망까지 왔다.
컨텍스트 이관 본체는 **아직이다** — FR-32가 `portfolio`·`market`·`news` 공개 API를 전제하는 것이
확인돼 REQ 본문을 정정했다(회고 §6). 새 서버 작업도 쓰기 시점에 `layer-check` 훅을 통과해야 한다.
