...
- 핵심 사용자 가치는 실시간 시장 확인, 내 포트폴리오 수익률 파악, 투자 위험 알림, 뉴스/심리 기반 판단 보조, AI 코치 해설이다.
- 현재 제품 방향은 단순 시세 앱이 아니라 “투자 초보도 자신의 투자 상태를 이해하고 행동할 수 있게 돕는 개인 투자 코치”에 가깝다.
- 우선순위는 `시장 조회 → 관심종목/실시간 가격 → 포트폴리오 → 인사이트/알림 → AI 코치/Gemini 해설 → 뉴스` 순서가 적합하다.
...
- 투자 후보군을 관리할 수 있다.
- 이후 알림, 인사이트, AI 코치와 연결되는 개인화 기반이 된다.
...
인사이트, AI 코치 판단 변화, 위험 신호 등 사용자가 놓치면 안 되는 투자 이벤트를 알림으로 제공하는 기능이다.
...
Should. 인사이트와 AI 코치가 사용자 행동으로 이어지려면 필요하다.

## 9. 투자 피드

### 기능 설명

인사이트, AI 코치 판단, 투자 알림을 한 곳에 모아 시간순으로 보여주는 기능이다.
...
사용자에게 직접 보이는 화면 기능이라기보다, 인사이트와 AI 코치를 만들기 위한 내부 분석 기반이다. 가격 히스토리와 RSI, 이동평균 같은 기술 지표를 주기적으로 계산한다.

### 사용자 이점

- 사용자가 직접 지표를 계산하지 않아도 분석 결과를 받을 수 있다.
- Smart Buy Zone, 과매수/과매도, 이동평균 교차 같은 판단 보조 기능의 기반이 된다.
...
Must for insight. 사용자가 직접 보는 기능은 아니지만 인사이트/AI 코치의 기반이다.
...
| 코치 과신 | AI 코치 해설을 확정 매매 조언처럼 오해할 수 있음 | “판단 보조/검토” 기능임을 명확히 표시 |
...
| 알림 읽음률 | 알림 품질이 충분한지 |
| AI 코치 해설 요청 수 | 사용자가 코치 판단 근거를 확인하는지 |
