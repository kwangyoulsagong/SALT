...
| 투자 시장 요약 띠 | Implemented | `apps/web` `widgets/market-board` `MarketSummaryStrip` (2026-09-22, `FE-REQ-037`) | `GET /api/app/market/summary` · WS `price_update.change24hAmount` | `GET /api/investment/market/summary` (설정 `MARKET_SUMMARY_*`) | `MarketAsset` 저장 시세 · 거래소 5분봉(1분 캐시) | 종목 · 태그 · 등락 금액은 서버가 정한다. 사유 태그 · 지수 · 일정 없음 |
...
| 거래 기록 · 계획 · 사이즈 계산 (F009) | Implemented (2026-09-24 슬라이스 3) | 종목 상세 `features/record-transaction` 거래 기록 카드(계획 접힘 · 서버 사이즈 결과 줄) · `entities/coach` "내 계획" 카드 | `POST /api/app/coach/trades`(거래 → 계획 조립) · `POST /size-check` · `GET · POST /plans` · `PATCH /plans/:id` | `POST /api/portfolio/transactions` · `/api/coach/size-check` · `/api/coach/plans` | `PortfolioTransaction`, `TradePlan`, `forecast.v_realized_vol` | 수동 입력 · 주문 없음 · 막지 않음. 손절 % 프리셋 · 손절까지 거리 보류. 실 토큰 확인 전 |
| 리스크 예산 게이지 (F009) | Implemented (2026-09-24 슬라이스 3) | 코치 리포트 `widgets/coach-console` 리스크 예산 패널 · `features/set-risk-budget` | `GET · PUT /api/app/coach/risk-budget` | `GET · PUT /api/coach/risk-budget` | `UserInvestmentProfile` 예산 3필드 | 이번 달 손실 · 종목 집중도 · 회전율. 포지션 화면이 없어 리포트에 둠. 매입가 숨김 화면 보류 |
...
| FEATURE-008 | AI 전망 · 인텔리전스 | 확률 · 구간 전망 + 채점 · 기준 대비 · 빗나간 사례(`ADR-003`). Python 예측 서비스(`ADR-004`). 변동 범위 · 해설 SSE · 주요 사건 구현됨(2026-09-24) |
| FEATURE-009 | 1인 펀드매니저 코치 | **2026-09-24 신설.** IPS · 리스크 예산 · 사이즈 계산 · 계획 기록 · 준수율 · 처분효과 · 벤치마크 · 실수 비용 · 월간 복기. 수동 입력 전제, 통제 · 차단 없음, 추가 입력 2개 이하 · 새 화면 0 |
