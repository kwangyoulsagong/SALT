...
| FR-28 | **Gemini 해설** (B3): [해설 보기]를 눌러야 부른다. ① 이 모드인 이유 ② 핵심 근거 ③ 주의점 ④ **관련 뉴스 5줄 요약** ⑤ 면책 + 같은 카드에 적중률 · 실패사례. **예상 수익 범위 없음**(서버에서 이미 제거 — `GeminiCoachExplainer`). **(2026-09-23)** LLM 이 만드는 범위는 계속 없다. 채점된 전망 구간 · 스트리밍 해설은 `FEATURE-008` FR-1~8 · FR-40~68. 판단이 미렌더면 버튼도 없고 LLM 도 부르지 않는다 | Must | Draft |
...
- **목표주가·수익률 예측 금지.** 가격은 내 규칙 기반(손절/익절)만. **(2026-09-23 `ADR-003`)** 확률 · 구간 전망은 `FEATURE-008` 에서 채점 3종과 함께 소유자에게만 연다. 한 점 목표가는 계속 금지.
...
| 2026-09-23 | 구현 진행 — **슬라이스 13 (서버) 재생성 쿨다운 · 기본 모드**: "다시 분석"이 즉시 받아지고(202) 뒤에서 돈다. 5분 안에 다시 누르면 남은 시간을 준다 — 화면은 `generation-status` 로 버튼을 언제 열지 안다. 설정의 **기본 모드가 저장되고** 모드를 고르지 않은 종목 판단이 그것을 따른다(B16). 근거 `requirements/reports/checklists/F004-server-coach-cooldown.md` |
| 2026-09-23 | 구현 진행 — **슬라이스 14 (BFF) 코치 리포트**: 코치 리포트 화면이 부를 경로(`/api/app/coach/report`)가 생겼다. 막힌 추천은 무엇을 추천했는지조차 보이지 않고 "왜 막혔는지 · 표본 몇 건"만 보인다. 다시 분석 버튼이 남은 시간을 받는다. 다음은 FE 화면. 근거 `requirements/reports/checklists/F004-bff-coach-report.md` |
