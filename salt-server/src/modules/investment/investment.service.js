...
const upbit_service_1 = require("./upbit.service");
const UPBIT_IMAGE_URL = "https://static.upbit.com/logos/";
class InvestmentService {
    upbitService = new upbit_service_1.UpbitService();
    /**
     * 관심 목록에 추가
     */
    async addToWatchlist(userId, data) {
        // 중복 확인
        const existing = await database_1.default.investmentWatchlist.findUnique({
            where: {
        ...
        if (existing) {
            throw new error_util_1.ConflictError("Already in watchlist");
        }
        // 실시간 가격 조회 (crypto만)
        ...
        // 관심 목록 추가
        const watchlist = await database_1.default.investmentWatchlist.create({
            data: {
                userId,
                assetType: data.assetType,
                symbol: data.symbol.toUpperCase(),
     ...
     * 관심 목록 조회
     */
    async getWatchlist(userId, query) {
        ...
        const [items, total] = await Promise.all([
            database_1.default.investmentWatchlist.findMany({
                where,
                skip,
                take: limit,
                orderBy: { addedAt: "desc" },
            }),
            database_1.default.investmentWatchlist.count({ where }),
        ]);
        // 로고 URL 추가
    ...
    async removeFromWatchlist(userId, watchlistId) {
        const item = await database_1.default.investmentWatchlist.findUnique({
            where: { id: watchlistId },
        });
        if (!item) {
            throw new error_util_1.NotFoundError("Watchlist item not found");
        }
        if (item.userId !== userId) {
            throw new error_util_1.NotFoundError("Watchlist item not found");
        }
        await database_1.default.investmentWatchlist.delete({
            where: { id: watchlistId },
        });
     ...
     * 차트 데이터 조회
     */
    async getChartData(symbol, period = "day", count = 30, unit = 5) {
        if (period === "day") {
    ...
    async getAllWatchlistSymbols() {
        const result = await database_1.default.investmentWatchlist.findMany({
     ...
     */
    async updateWatchlistPrices(priceData) {
        const updates = priceData.map((data) => database_1.default.investmentWatchlist.updateMany({
            where: {
                symbol: data.symbol.toUpperCase(),
                assetType: "crypto",
            },
     ...
     */
    async getMarketOverview(query) {
        const page = query.page && query.page > 0 ? query.page : 1;
        const limit = query.limit && query.limit > 0 ? query.limit : 100;
        const sort = query.sort || "trade_value";
        ...
        try {
            ...
            // DB에서 페이징 처리하여 조회
            const [items, total] = await Promise.all([
                database_1.default.marketAsset.findMany({
                    where,
                    orderBy,
                    skip: (page - 1) * limit,
                    take: limit,
                }),
                database_1.default.marketAsset.count({ where }),
            ]);
            ...
            const overview = items.map((item) => ({
                ...
                logoUrl: item.logoUrl || `https://static.upbit.com/logos/${item.symbol}.png`,
     ...
     * 응답을 기다리지 않고 비동기로 실행
     */
    async updateMarketPricesInBackground(symbols) {
        try {
            // 10분 이내에 업데이트된 건 스킵
            const TEN_MINUTES = 10 * 60 * 1000;
            const now = new Date();
            // 업데이트가 필요한 심볼만 필터
            const needsUpdate = await database_1.default.marketAsset.findMany({
                where: {
                    symbol: { in: symbols },
                    OR: [
                        { priceUpdatedAt: null },
            ...
            // DB 업데이트 (배치)
            const updates = prices.map((price) => database_1.default.marketAsset.update({
    ...
    async updateAllMarketPrices() {
        try {
            // 모든 활성 마켓 조회
            const markets = await database_1.default.marketAsset.findMany({
                where: { isActive: true },
                select: { symbol: true },
            });
            ...
            for (let i = 0; i < symbols.length; i += BATCH_SIZE) {
                ...
                await Promise.all(updates);
                console.log(`Batch ${i / BATCH_SIZE + 1}: Updated ${updates.length} prices`);
                // API 제한 방지
     ...
     */
    sortMarketOverview(overview, sort, order) {
     ...
     */
    marketCache = new Map();
    CACHE_TTL = 60 * 1000; // 1분
    async getCachedMarketOverview(query) {
    ...
    async getAllMarketSymbols() {
        try {
            const rows = await database_1.default.marketAsset.findMany({
