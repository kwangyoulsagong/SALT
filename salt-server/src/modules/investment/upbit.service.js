...
class UpbitService {
    ...
    async getCurrentPrice(symbol) {
        try {
            ...
            const response = await axios_1.default.get(`${UPBIT_API_URL}/ticker`, {
            ...
            logger_1.logger.error("Upbit API error:", error.message);
     ...
     * 일봉 차트 데이터 조회
     */
    async getDailyCandles(symbol, count = 30) {
        try {
            const market = `KRW-${symbol.toUpperCase()}`;
            const response = await axios_1.default.get(`${UPBIT_API_URL}/candles/days`, {
                params: { market, count },
            });
            ...
            logger_1.logger.error("Upbit candles API error:", error.message);
     ...
     * 분봉 차트 데이터 조회
     */
    async getMinuteCandles(symbol, unit, count = 200) {
        try {
            const market = `KRW-${symbol.toUpperCase()}`;
            const response = await axios_1.default.get(`${UPBIT_API_URL}/candles/minutes/${unit}`, { params: { market, count } });
            return response.data.map((candle) => ({
            ...
            logger_1.logger.error("Upbit minute candles API error:", error.message);
    ...
    async getAllMarkets() {
        try {
            const response = await axios_1.default.get(`${UPBIT_API_URL}/market/all`);
            return response.data.filter((m) => m.market.startsWith("KRW-"));
        }
        catch (error) {
            logger_1.logger.error("Upbit markets API error:", error.message);
    ...
    async getCurrentPrices(symbols) {
        try {
            ...
            logger_1.logger.error("Upbit ticker API error:", error.message);
     ...
     */
    async getBatchCandles(symbols, period) {
        const days = this.getPeriodDays(period);
        const BATCH_SIZE = 10; // 동시 실행 제한 (Upbit API 제한 고려)
        const results = {};
        try {
            logger_1.logger.info(`Starting batch candle fetch for ${symbols.length} symbols, period: ${period}`);
            // 심볼을 배치 단위로 나누어 처리
            for (let i = 0; i < symbols.length; i += BATCH_SIZE) {
                const batch = symbols.slice(i, i + BATCH_SIZE);
                // 현재 배치를 병렬로 처리
                const promises = batch.map((symbol) => this.getDailyCandles(symbol, days + 1)
                    .then((data) => ({ symbol, data, success: true }))
                    .catch((error) => {
                    logger_1.logger.warn(`Failed to fetch candles for ${symbol}:`, error.message);
                ...
                logger_1.logger.debug(`Processed ${processed}/${symbols.length} symbols`);
            }
            logger_1.logger.info(`Batch candles fetched successfully for ${Object.keys(results).length} symbols`);
            return results;
        }
        catch (error) {
            logger_1.logger.error("Batch candles fetch error:", error.message);
            // 부분 실패해도 가져온 데이터는 반환
            return results;
        }
    }
    ...
    async getCachedDailyCandles(symbol, count = 30) {
        const cacheKey = `${symbol}-${count}`;
        const cached = this.candleCache.get(cacheKey);
        // 캐시가 유효하면 반환
        if (cached && Date.now() - cached.timestamp < this.CACHE_TTL) {
            logger_1.logger.debug(`Cache hit for ${symbol} candles`);
            return cached.data;
        }
        // 새로 조회
    ...
    async getCandlesByTimeframe(symbol, timeframe, count) {
        switch (timeframe) {
            case "1d":
...
exports.UpbitService = UpbitService;
//# sourceMappingURL=upbit.service.js.map