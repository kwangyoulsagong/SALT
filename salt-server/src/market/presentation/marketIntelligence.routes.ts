...
/** 시장 심리·스마트머니 경로. `/api/market-intelligence` 를 그대로 유지한다 (FR-35). */
export const createMarketIntelligenceRouter = (
    useCases: MarketUseCases
  ): Router => {
  const router = Router();
  const controller = new MarketIntelligenceController(useCases);
