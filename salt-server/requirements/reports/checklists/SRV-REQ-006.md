...
> **정정 (2026-09-21) — 이 표는 한 번의 기동만 본 것이다.** 같은 날 뒤이은 콜드 스타트
> 실측은 429 **985건**(재시도 1회 607 · 2회 361 · 3회 17) · 캔들 수집 실패 **2건**(STX 1d ·
> POLYX 5m)이었다. "0건"은 운이 좋았던 회차였다. 실제로 0건을 만든 것은 §15 다.
...
| 기동 + `/health` | 200 · 캔들 수집 실패 0건 — **이 회차만 그랬다.** 재현되는 0건은 §15 |
...
| Worker/External | **pass** | 단계 격리(§14-2) · 캔들 수집 실패 0건 **(§15 이후 — 그 전에는 회차에 따라 2건)** · 코치 단계 실행 확인 |
| 정적 점검 | **pass** | `presentation` → `prisma` 0건 · `domain` 금지 패키지 0건(주석 1건은 설명) · `application` → `infrastructure` 0건(주석 6건은 설명) |

## 15. 거래소 레이트리밋을 보내기 전에 지킨다 (2026-09-21)

§12-3 의 "캔들 수집 실패 0건"이 **재현되지 않았다.** 재시도는 이미 맞은 429 를 줍는
장치라, 호출 쪽이 동시에 던지는 한 0건을 보장하지 못한다 — `CollectPriceHistory` 가
심볼 10개를 `Promise.all` 로 던지고, 재시도도 같이 몰렸다.

**고친 자리는 수집 유스케이스가 아니라 `UpbitClient` 다.** 거래소를 부르는 유스케이스가
넷이고(`market-sync` · 시세 갱신 · 캔들 수집 · 시장 인텔리전스) 한도는 **프로세스 전체**의
것이다. 한 유스케이스의 동시성을 줄이면 나머지 셋이 같은 한도를 모르고 쓴다.

- `shared/infrastructure/pacer.ts` — 출발 간격 제한(`createRatePacer`). 테스트 5건
- `UpbitClient` — 거래소 그룹 둘(캔들 · 그 외)마다 초당 **8건**(한도 10, `Remaining-Req:
  group=candles; min=600; sec=9` 로 확인). 페이서는 **재시도 안쪽**에 있다 — 재시도도 한 건이다

| 콜드 스타트 | Upbit 429 | 캔들 수집 실패 | 관찰 시간 |
|---|---|---|---|
| 2026-09-18 (페이서 없음) | 985 | 2 | 기동 버스트 |
| 1초 창에 8건 **묶음** 출발 | 27 (전부 재시도 1회에 회복) | 0 | 150초 |
| **125ms 간격** 출발 | **0** | **0** | 170초 |

묶음 방식이 0 이 아니었던 이유: 창이 열리는 순간 8건이 동시에 나가고, 응답 지연 편차로
거래소 쪽에서는 앞 창의 꼬리와 뒤 창의 머리가 같은 1초에 도착한다. 간격으로 펴면 지연
편차가 125ms 를 넘어야 몰린다.

**대가**: 캔들 수집 한 회차가 약 70초 걸린다(활성 크립토 약 280개 × 2 타임프레임 ÷ 8).
주기(5분)보다 짧다. 시세 갱신(분당 3건)과 시장 인텔리전스는 다른 그룹이라 캔들 수집
뒤에 줄 서지 않는다.

**남은 것**: 같은 IP 에서 다른 프로세스가 거래소를 부르면 이 프로세스는 그 몫을 모른다
(한도보다 2건 낮게 잡은 이유). 서버 인스턴스를 둘 띄우는 배포가 오면 다시 본다.
