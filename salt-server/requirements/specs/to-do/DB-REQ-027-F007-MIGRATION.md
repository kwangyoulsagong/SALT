...
전부 **신규 테이블**이다. 기존 행을 변형하지 않으므로 백필이 없고 되돌리기가 쉽다. 유일한 주의점은 **`User`에 역관계 2개 추가**(2026-09-21 — `NotificationPreference` 삭제)와 **기존 `InvestmentNotification`과의 관계 정리**다.
...
CREATE TYPE "NotificationType" AS ENUM ('SIGNAL_UPDATE');          -- 개정 2026-09-21: TAX_DEADLINE 삭제 (ADR-002 · B21)
...
### M4 — `app_version_gates`

*(개정 2026-09-21 — `notification_preferences` 삭제. 알림 on/off는 F006 `User.alertsEnabled`, `DB-REQ-023` 마이그레이션 소관)*
...
| FR-5 | 롤백은 `DROP TABLE` 3개 + `DROP TYPE` 5개다. 데이터 손실 범위가 신규 테이블로 한정된다. *(개정 2026-09-21 — 테이블 4→3)* | Must |
| FR-6 | **기존 `InvestmentNotification`을 건드리지 않는다.** F006이 그 테이블을 알림 목록 원천으로 쓰고, 이 REQ의 `NotificationDelivery.payloadRef`가 그 id를 가리킨다(FK 없음) | Must |
...
| 단계 | 검사 |
|---|---|
| 마이그레이션 후 | 3개 테이블·5개 enum·6개 인덱스 존재 (2026-09-21 개정) |
...
| ID | 요구사항 |
|---|---|
| FR-20 | 롤백 시 `notification_deliveries`가 사라진다 → 롤백 직후 같은 알림이 한 번 더 푸시될 수 있다(dedupe 기록 손실). 롤백 전 덤프를 뜬다. *(개정 2026-09-21 — "세금 시즌 롤백 금지"는 ADR-002로 근거 소멸)* |
| FR-21 | ~~12월 이전 배포 · F002 하드 마감 선행~~ → **삭제 2026-09-21** — F002가 없다(ADR-002). 하드 마감 없음 |
...
- [ ] **백필이 0건이다**
- [ ] 3개 테이블·5개 enum·6개 인덱스가 생성된다 (2026-09-21 개정)
- [ ] `NotificationType` enum이 `SIGNAL_UPDATE` 하나다 (B21)
...
- [ ] 롤백 전 `notification_deliveries` 덤프가 릴리스 체크리스트에 있다 (2026-09-21 개정)

## Dependencies

- **선행:** `DB-REQ-025`(SCHEMA) · `DB-REQ-026`(FUNC)
- **짝:** `DB-REQ-028`(PERF)
- **연관:** `DB-REQ-021`·`023`(F006 — `InvestmentNotification` · `User.alertsEnabled`). *(개정 2026-09-21 — "F002 세금 D-Day 차단" 삭제, ADR-002)*

## Open Questions

- ~~`InvestmentNotification`과 `NotificationDelivery` 통합~~ → **정리 2026-09-21** — 목록은 `InvestmentNotification`만 읽는다(F006). `NotificationDelivery`는 푸시 기록이라 합칠 필요가 없다.
...
| 날짜 | 변경 |
|---|---|
| 2026-09-21 | `pm/requirements/reports/feature-audits/2026-09-21-storyboard-gap.md` · `ADR-002` 반영. `NotificationPreference` 테이블 삭제(M4), enum `SIGNAL_UPDATE` 1종(B21). FR-5 · FR-6 · FR-20 개정, FR-21(F002 하드 마감) 삭제 |
