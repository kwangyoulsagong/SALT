...
import { INVESTMENTS_BASE_URL, END_POINTS } from "@/constants/api";
...
export const investmentsAPi = {
  marketOverview: async (
    ...
    const data = await axios.get(
      `${INVESTMENTS_BASE_URL}${END_POINTS.marketOverview(params)}`
    );
    return data.data;
  },
  marketChartPreview: async (
    ...
    const data = await axios.get(
      `${INVESTMENTS_BASE_URL}${END_POINTS.marketChartPreview(symbol)}`
    );
    return data.data;
  },
  marketIntelligencePreview: async (
    ...
    const data = await axios.get(
      `${INVESTMENTS_BASE_URL}${END_POINTS.marketIntelligencePreview(symbol)}`
    );
    return data.data;
  },
