...
class WSClient {
  ...
  private connect() {
    ...
    this.ws.onopen = () => {
      ...
      if (priceSymbols.length > 0) {
        this.send({ type: WSMessageType.Subscribe, symbols: priceSymbols });
      }
  ...
  subscribePriceBatch(symbols: string[]) {
    ...
    if (!this.reconnecting) {
      this.send({ type: WSMessageType.Subscribe, symbols: upper });
    }
  ...
  unsubscribePriceBatch(symbols: string[]) {
    const upper = symbols.map((s) => s.toUpperCase());
    if (!this.reconnecting) {
      this.send({ type: WSMessageType.Unsubscribe, symbols: upper });
    }
    upper.forEach((symbol) => {
  ...
  unsubscribeCandle(
    ...
    if (set.size === 0) {
      ...
      this.send({ type: WSMessageType.UnsubscribeCandle, symbol: upper, timeframe });
    }
