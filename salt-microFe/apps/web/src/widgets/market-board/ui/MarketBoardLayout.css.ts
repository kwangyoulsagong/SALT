...
export const splitLayout = style({
  ...
  "@media": {
    ...
    [DESKTOP]: {
       ...
       * **화면에 붙인다** (2026-09-23).
       *
       * 상자 높이가 뷰포트 기준인데 위치는 문서 기준이라, 페이지가 스크롤된 상태에서는
       * 상자가 화면 밖으로 밀려 표 머리와 패널 위쪽이 잘렸다. 상세에서 돌아올 때 브라우저가
       * 스크롤 위치를 복원하면 **항상** 그 상태가 됐다(사용자 보고 — "돌아가면 깨져").
       *
       * 붙여 두면 헤더가 밀려 올라간 뒤 상자가 화면을 채우고, 스크롤 위치가 어디든 같은
       * 모습이 된다. 원래 의도(헤더는 스크롤로 밀고 표 · 패널은 한 화면에)와 같은 동작이다.
       */
      position: "sticky",
      top: 0,
      alignItems: "stretch",
