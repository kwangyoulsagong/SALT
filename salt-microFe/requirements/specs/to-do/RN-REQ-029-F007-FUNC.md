...
| ID | 요구사항 | 우선순위 |
|---|---|---|
| FR-10 | 알림 탭 → `PushPayload`(`type: 'signal_update'`, `alertId`, `target`, `symbol?`)로 라우팅한다. 웜·콜드 모두. **개정 2026-09-21** — 푸시 1종(D5 · 기본안 — 감사 문서 B21), 문자열 `deepLink` 대신 `target`(`BFF-REQ-032`) | Must |
| FR-17 | **2026-09-21 추가.** 라우팅 표: `target: 'plan'` → 적립 상세(`PlanDetailScreen`) · `'coach'` → 코치 탭 추천 카드 · `'detail'` → 상세 분석 페이지(`symbol`). 세금 · 청구서 대상이 0건이다(ADR-002) | Must |
| FR-18 | **2026-09-21 추가.** 푸시로 진입하면 그 알림을 읽음 처리한다(F006 `PATCH /api/app/alerts/:id/read`) | Should |
...
| ID | 요구사항 | 우선순위 |
|---|---|---|
| FR-20 | 홈 응답(3블록 — D6)을 캐시하고 오프라인에서 **`lastUpdatedAt`과 함께** 표시한다. 기존 블록(목표 카드 등)도 마지막 응답을 쓴다 | Must |
...
**왜 큐가 없는가** (FEATURE-007 FR-32): 나중에 자동 전송되면 사용자가 결과를 확인하지 않은 채 상태가 바뀐다. 적립 완료 · 거래 기록 · 평가는 **사실 기록**이므로 사용자가 결과를 본 상태에서만 확정한다.
...
| FR-43 | **금액 계산 · 표시가 바뀌는 릴리스는 즉시 적용 + 재시작 안내**를 한다. 다음 실행까지 미루지 않는다. *(개정 2026-09-21 — "세금 계산" 삭제, ADR-002)* | Must |
...
| FR-51 | **거래소 API 키를 앱에 저장하는 경로가 0건**이다(계좌 연동은 영구 Non-Goal — ADR-002) | Must |
| FR-52 | **앱이 로컬 알림을 스케줄하지 않는다** | Must |
| FR-53 | 오프라인 캐시·로그·크래시 리포트에 **금액이 0건**이다 | Must |
| FR-54 | `src/native/` 밖에서 플랫폼 모듈 import가 0건이다 | Must |
| FR-55 | **파일 접근 권한을 요청하지 않는다.** **개정 2026-09-21** — CSV 업로드 삭제(ADR-002 · 계좌 연동 Non-Goal). 보유 기록은 거래 기록 시트로 직접 입력한다(B14) | Must |
...
| 기기 | iOS·Android 실기기 푸시 도달 → 딥링크 (`target` 3종) |
| 정적 | 세금 · 청구서 라우트 · 딥링크 문자열 0건 (2026-09-21) |
...
- [ ] 알림 탭이 웜·콜드 모두 `target` 3종으로 라우팅된다 (D5 · B21)
- [ ] **세금 · 청구서 라우트와 딥링크가 0건이다** (ADR-002)
- [ ] **딥링크 파라미터가 서버로 재검증된다**
- [ ] **외부 URL 딥링크가 거부된다**
...
| 날짜 | 변경 |
|---|---|
| 2026-09-21 | `pm/requirements/reports/feature-audits/2026-09-21-storyboard-gap.md` · `ADR-002` 반영. 딥링크를 `PushPayload.target` 3종으로 개정(FR-10, D5 · B21), FR-17 · 18(라우팅 표 · 읽음) 추가, FR-43 · 55(세금 · CSV 삭제, ADR-002) · FR-20(홈 3블록, D6) 개정 |
