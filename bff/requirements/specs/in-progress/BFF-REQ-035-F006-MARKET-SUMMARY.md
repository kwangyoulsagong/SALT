...
| FR-2 | 뷰모델: `name`(한글 이름, 없으면 심볼) · 모르는 태그 코드 버림 · 점 2개 미만 스파크라인은 `null` · `breadth` 합 0 이면 `null` · `headlines` 는 카드 필드만(옛 서버면 빈 배열) · 고가/저가/거래대금 그대로. **계산 없음** | Must |
...
- 2026-09-22 개정 — `breadth` · `headlines` · 고가/저가/거래대금 전달
