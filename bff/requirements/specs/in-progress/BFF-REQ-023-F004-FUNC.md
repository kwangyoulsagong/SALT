...
| 2026-09-23 | **슬라이스 14 구현.** `/api/app/coach/report`(FR-10~14 — 막힌 추천은 판별 union 으로 접어 행동 · 종목 · 점수를 떨군다) · `/api/app/coach/generation-status`(FR-63) · 429 본문 `retryAfterSeconds` 전달(FR-60 — 에러 미들웨어가 떨구고 있었다) · 행동 기록 `factCode`/`params`(FR-40 · 41). FR-11 해석: `degradedFields` 는 **계약이 깨진 필드**다 — 서버의 `recommendation: null` 은 추천 없음이다. 근거 `reports/checklists/BFF-REQ-023.md` |
| 2026-09-23 | FE 소비 시작(F004 슬라이스 15 — `/coach/report` 화면). BFF 변경 없음. **FE 가 `candidates` 를 읽지 않는다** — 3종 세트 없는 추천이고 막힌 추천의 종목이 후보 1위로 남는다. 게이트 붙은 후보 계약 또는 필드 제거를 PM 과 정한다(`FEATURE-004` 변경 이력). 참고: 서버 후보에는 `reasons` 가 없는데 뷰모델 타입은 `reasons: string[]` 이다 |
